...
int main(int argc, char **argv) {
    ...
    auto build = [&](const char *tag, bool vec_obs) {
        ...
        y(i, b, n) = select(n <= 1, base,
                            select(off == 0, onceper + diff, y(i, b, n) + diff));
        ...
        y.update(0).allow_race_conditions().unroll(r.y).vectorize(r.x, 16);
        E.bound(b, 0, B).bound(n, 0, T);
        E.compile_jit();
        return E;
    };

    Func E_scalar = build("scalar", false);
    Func E_vec = build("vec", true);
