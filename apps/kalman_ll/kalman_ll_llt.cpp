...
int main(int argc, char **argv) {
    ...
    auto build = [&](bool inductive) -> Func {
        ...
        if (inductive) {
            State.reorder_storage(b, c, t).compute_at(LL, rl).store_at(LL, bo)
                 .fold_storage(t, 2).vectorize(b, V);
        } else {
    ...
    try {
        ...
        li.compile_jit(); ln.compile_jit(); lf.compile_jit();
        Buffer<double> ri(B), rn(B), rf(B);
        li.realize(ri); ln.realize(rn); lf.realize(rf);
        ...
        if (fp) { fprintf(fp, "%d %d %.6f %.6f %.9f %.9f %.9f\n", B, T, ti, tn, q0, q1, Rv); fclose(fp); }
