#!/usr/bin/env python3
# FAIR compiled+batched third-party baseline for the ar_ll workload: JAX with a
# JIT-compiled lax.scan over time and vmap over the batch (what dynamax / TFP do
# internally). CPU backend, so it's compiled-CPU-vs-compiled-CPU against Halide.
...
z = np.fromfile("z_ar.bin", dtype=np.float64).reshape(T, B).T   # (B, T)
ll_halide = np.fromfile("ll_ar.bin", dtype=np.float64)          # (B,)

jax.config.update("jax_enable_x64", True)

F = jnp.array([[phi1, phi2], [1.0, 0.0]])
H = jnp.array([[1.0, 0.0]])
Q = jnp.array([[q, 0.0], [0.0, 0.0]])
R = jnp.array([[Rv]])
x0 = jnp.zeros((2, 1))
P0 = jnp.eye(2)
