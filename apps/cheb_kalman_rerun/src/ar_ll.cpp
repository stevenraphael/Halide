...
int main(int argc, char **argv) {
    ...
    auto build = [&](bool inductive, int fold_k = 2) -> Func {
        ...
        if (inductive) {
            ...
            State.update(0).unroll(rc);
        } else {
            State.reorder_storage(b, c, t).compute_at(LL, bo).vectorize(b, V);
            State.update(0).reorder(b, r.x, r.y).vectorize(b, V).unroll(r.x);
        }
        return LL;
    };
    ...
    try {
        ...
        // allocator; outside the timed benches above). Reports the actual bytes
        // Halide allocates for the recurrence-state scratch, not an analytic model.
        const double meas_non = hb::measure_jit_peak(ln, [&] { ln.realize(rn); });
        const double meas_unf = hb::measure_jit_peak(lu, [&] { lu.realize(ru); });
        const double meas_ind = hb::measure_jit_peak(li, [&] { li.realize(ri); });
