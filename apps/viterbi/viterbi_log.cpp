...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Buffer<int> obs(T);
        for (int i = 0; i < S; i++) init(i) = 1.0f / S;
        for (int r = 0; r < S; r++) {
            float row_sum = 0;
            for (int c = 0; c < S; c++) { trans(r, c) = 1 + ((r + 1) * (c + 2)) % 5; row_sum += trans(r, c); }
            for (int c = 0; c < S; c++) trans(r, c) /= row_sum;
        }
        for (int st = 0; st < S; st++) {
            float row_sum = 0;
            for (int o = 0; o < M; o++) { emit(st, o) = 1 + ((st + 1) * (o + 3)) % 4; row_sum += emit(st, o); }
            for (int o = 0; o < M; o++) emit(st, o) /= row_sum;
        }
        for (int i = 0; i < T; i++) obs(i) = (i * 2 + 1) % M;
        ...
        // Build one variant; returns the decoded `path` Func.
        ...
        Func path_i = build(true), path_n = build(false);
        Buffer<int> res_i(T), res_n(T);
        ...
        for (int st = 0; st < S; st++) rv[0][st] = log_init(st) + log_emit(st, obs(0));
        for (int tt = 1; tt < T; tt++)
            for (int st = 0; st < S; st++) {
                float best = neg_inf; int best_r = 0;
                for (int rr = 0; rr < S; rr++) {
                    float v = rv[tt - 1][rr] + log_trans(st, rr) + log_emit(st, obs(tt));
                    if (v >= best) { best = v; best_r = rr; }
                }
                rv[tt][st] = best; rp[tt][st] = best_r;
            }
        std::vector<int> ref_path(T);
        { int best_r = 0; float best = neg_inf;
          for (int st = 0; st < S; st++) if (rv[T - 1][st] >= best) { best = rv[T - 1][st]; best_r = st; }
          ref_path[T - 1] = best_r; }
        for (int tt = T - 2; tt >= 0; tt--) ref_path[tt] = rp[tt + 1][ref_path[tt + 1]];

        auto count_mismatch = [&](Buffer<int> &res) {
            int m = 0; for (int tt = 0; tt < T; tt++) if (res(tt) != ref_path[tt]) m++; return m; };
        int mi = count_mismatch(res_i), mn = count_mismatch(res_n);

        const double bytes_ind = (double)S * 2 * 4;
        const double bytes_non = (double)S * T * 4;
        char note[128];
        snprintf(note, sizeof(note), "Viterbi decode (log domain)  S=%d M=%d T=%d  |  state fold %.0fx",
                 S, M, T, hb::mem_ratio(bytes_non, bytes_ind));
        hb::print_spec_header("viterbi_log", "host", note);
        hb::print_row("non-inductive (materialize)", sn, (S * (double)T) / (sn.median * 1e3),
                      "Mstates/s", bytes_non, (double)mn, mn == 0);
        hb::print_row("inductive (fold t -> 2)", si, (S * (double)T) / (si.median * 1e3),
                      "Mstates/s", bytes_ind, (double)mi, mi == 0, "win");

        // Dump the inductive path for the librosa comparison script.
        std::vector<int32_t> po(T);
        for (int i = 0; i < T; i++) po[i] = res_i(i);
        FILE *f = fopen((std::string(data_path) + ".path").c_str(), "wb");
        if (f) { fwrite(po.data(), sizeof(int32_t), T, f); fclose(f); }

        return (mi == 0 && mn == 0) ? 0 : 1;
