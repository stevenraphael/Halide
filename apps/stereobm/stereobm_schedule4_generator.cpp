...
//  (BSD-style OpenCV license -- see stereobm_generator.cpp for full text.)
//
// M*/

// Halide adaptation of https://github.com/opencv/opencv/blob/4.x/modules/calib3d/src/stereobm.cpp
//
// "Schedule 4" variant of stereobm_tiled_generator.cpp. It differs from the
// Schedule 3 tiled version in two ways:
//
//   1. The disparity axis d is split into an inner index di (of width
//      vector_width) and an outer index d_o, with di made the innermost
//      storage/loop dimension so the cost-aggregation funcs vectorize over
//      disparity (di) rather than over x.
//
//   2. BOTH box-filter passes are sliding RDom scans: vsum slides the vertical
//      window down each y tile, and cSAD slides the horizontal window across
//      each x tile. (Schedule 3 kept the horizontal pass as a redundant sum().)
//      This makes the runtime window-size invariant, like the inductive version.
//
// The disadvantage, per the paper, is memory: vsum and cSAD each need
// vector_width * x_tile_size * y_tile_size storage instead of
// x_tile_size * y_tile_size. The output is bit-for-bit identical to
// stereobm_generator.
...
public:
    ...
    void generate() {
        ...
        // disparity split: full disparity = d_o * VW + di, di in [0, VW).
        ...
        // Explicit (xi, yi, xo, yo) tiling, plus (di, d_o) disparity split.
        Expr ax = xi + xo * T;  // absolute x
        Expr ay = yi + yo * N;  // absolute y

        // diff indexed by the *full* disparity d (0..depth-1).
        Func diff("diff");
        Var d("d");
        ...
        Expr d_full = d_o * VW + di;  // disparity handled by this (di, d_o) lane

        // vsum: vertical window sum of diff, as a sliding scan down each y tile.
        ...
        // cSAD: horizontal window sum of vsum, as a sliding scan across each x tile.
        ...
        // Read the SAD for an arbitrary full disparity value (used by argmin scans /
        // subpixel gather). d_expr is 0..depth-1.
        auto sad = [&](Expr d_expr, Expr X, Expr Y) {
            return cSAD(d_expr % VW, X, Y, xo, yo, d_expr / VW);
        };

        // texture: 2D box sum of the prefiltered left image (no disparity dependence).
        Func text("text");
        ...
        // 2D reduction over disparity: rd[0] = di (inner/vector lane), rd[1] = d_o.
        ...
        // disp_left == the paper's disp_left: vectorize over xi, tile loops (xo, yo) parallel.
        ...
        // Disparity search: vectorize the inner disparity lane rd[0], unroll d_o (rd[1]).
        ...
        // Cost aggregation: di innermost and vectorized; both passes are scans.
