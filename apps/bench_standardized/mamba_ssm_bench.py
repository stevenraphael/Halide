...
print(
    f"Mamba SOTA fused CUDA kernel ({torch.cuda.get_device_name(0)})  N={N} D={D} T={T} batch={batch}"
)
print(
    f"  mamba_ssm selective_scan_fn (fused CUDA): {t_fused:8.3f} ms  {mtoks:8.1f} Mtok/s   (rel vs ref {rel:.1e})"
)
