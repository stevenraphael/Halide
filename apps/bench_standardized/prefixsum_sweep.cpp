...
double time_rdom(int W, int H) {
    ...
    return hb::bench([&]() { output_a.realize(result_a); }).min / 1e3;  // ms -> s
}
...
double time_inductive(int W, int H, int fold) {
    ...
    return hb::bench([&]() { output_b.realize(result_b); }).min / 1e3;  // ms -> s
}
...
int main(int argc, char **argv) {
    ...
    if (argc > argi) {
        Ws.clear();
        for (; argi < argc; argi++)
            Ws.push_back(atoi(argv[argi]));
    }

    hb::print_spec_header("prefixsum_sweep", "host",
                          "W-sweep: RDom (materialize) vs inductive fold=1 vs fold=2 (median ms)");
