...
def main():
    ...
    print(f"numpy     (W={W}, H={H}): best of {trials} = {best:.3f} ms "
          f"({W * H / best / 1000.0:.2f} Mpixels/s)")

    n_mismatch = int(np.sum(out != halide_out))
    print(f"Output mismatch vs Halide: {n_mismatch} / {W * H} "
    ...
    print(f"numba     (W={W}, H={H}): best of {trials} = {best_nb:.3f} ms "
          f"({W * H / best_nb / 1000.0:.2f} Mpixels/s)")

    n_mismatch_nb = int(np.sum(out_nb != halide_out))
    print(f"Output mismatch vs Halide: {n_mismatch_nb} / {W * H} "
