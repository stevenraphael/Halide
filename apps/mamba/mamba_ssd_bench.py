...
batch = int(sys.argv[5]) if len(sys.argv) > 5 else 16   # match the batched inductive test
dev = "cuda"
dtype = torch.float32
torch.manual_seed(5)
...
print(f"Mamba-2 SSD (Triton) fused kernel  N={N} D={D} T={T} batch={batch}  "
      f"(nheads={nheads} headdim={headdim} dstate={dstate})  ({name})")
print(f"  {'mamba_chunk_scan_combined':30s}  {med:10.3f} {p25:10.3f} {p75:10.3f}   {mtoks:10.1f}  (median/p25/p75 ms, Mtok/s)")
