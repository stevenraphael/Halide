...
int main(int argc, char **argv) {
    ...
    auto run_for_batch = [&](int Bsz) {
        ...
        auto build = [&](bool inductive) -> Func {
            ...
            if (inductive) {
                ...
                h.compute_root();
                h.split(d, dob2, dib2, 64).reorder(t, n, dib2, dob2, bb)
                 .gpu_blocks(dob2, bb).gpu_threads(dib2);
                h.update(0).split(d, dob2, dib2, 64).reorder(n, dib2, dob2, bb, rt)
                 .gpu_blocks(dob2, bb).gpu_threads(dib2);
            }
            Func y("y_out");
            Expr acc = 0.0f;
            for (int nn = 0; nn < N; nn++) acc += c(nn, t, bb) * h(d, nn, t, bb);
            y(d, t, bb) = acc;

            Var dob("dob"), dib("dib"), hb_("hb"), ht_("ht");
            y.bound(d, 0, D).bound(t, 0, T).bound(bb, 0, Bsz)
             .split(d, dob, dib, 64)
             .reorder(t, dib, dob, bb)          // t serial innermost
             .gpu_blocks(dob, bb).gpu_threads(dib);   // lanes = batch * (D/64)*64
            if (inductive) {
                h.compute_at(y, t).store_at(y, dib).unroll(n).fold_storage(t, 1);
        ...
        if (do_check)
            for (int q = 0; q < Bsz; q++)
                for (int dd = 0; dd < D; dd++) {
                    ...
                    for (int nn = 0; nn < N; nn++) hs[nn] = delta(dd, 0, q) * b(nn, 0, q) * x(dd, 0, q);
                    float y0 = 0; for (int nn = 0; nn < N; nn++) y0 += c(nn, 0, q) * hs[nn];
        ...
        for (int mode = mode0; mode < 2; mode++) {
            ...
            try {
                ...
                f.realize(r, target); r.copy_to_host();
                double err = -1.0;
                if (do_check) {
                    err = 0;
                    for (int q = 0; q < Bsz; q++)
                ...
                double mtoks = toks / (s.median * 1e3);
    ...
    std::vector<int> batches = (argc > 4) ? std::vector<int>{atoi(argv[4])}
                                          : std::vector<int>{1, 2, 4, 8, 16, 32};
    for (int Bsz : batches) run_for_batch(Bsz);
    return 0;
}
