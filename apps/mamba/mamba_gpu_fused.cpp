...
int main(int argc, char **argv) {
    ...
    auto build = [&](bool inductive) -> Func {
        ...
        y.bound(d, 0, D).bound(t, 0, T)
         .split(d, dob, dib, 64)
         .reorder(t, dib, dob)          // t innermost (serial), then thread, block
        ...
        if (inductive) {
            ...
            h.compute_at(y, t).store_at(y, dib)
             .unroll(n).fold_storage(t, 2);
        } else {
    ...
    try {
        ...
        for (int mode = 0; mode < 2; mode++) {
            ...
            double err = 0;
            for (int dd = 0; dd < D; dd++) for (int tt = 0; tt < T; tt++)
                err = std::max(err, (double)std::abs(r(dd, tt) - cy[(size_t)dd * T + tt]));
            double best = 1e18;
            for (int q = 0; q < 5; q++) {
                auto t0 = std::chrono::high_resolution_clock::now();
                f.realize(r, target); r.device_sync();
