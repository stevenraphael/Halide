...
print(
    f"Mamba GPU third-party (PyTorch, {torch.cuda.get_device_name(0)})  N={N} D={D} T={T}  L={L}"
)
print(f"  PyTorch sequential (selective_scan_ref style): {t_seq:8.3f} ms")
print(
    f"  PyTorch chunked-parallel (sqrt-T, torch ops):  {t_par:8.3f} ms   (rel seq-vs-par {rel:.1e})"
)
