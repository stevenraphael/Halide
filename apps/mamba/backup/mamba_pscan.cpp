...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < T; t++) {
        for (int n = 0; n < N; n++) {
            ...
            for (int d = 0; d < D; d++) a(d, n, t) = 0.9f + (rand() % 99) / 1000.0f;
        }
        for (int d = 0; d < D; d++) x(d, t) = (rand() % 200) / 100.0f - 1.0f;
    }
    ...
    auto build = [&](bool inductive) -> Func {
        ...
        if (inductive) {
            ...
            carry(d, n, k) = select(k <= 0, 0.0f,
                                    likely(PA(d, n, k - 1, L - 1) * carry(d, n, k - 1)
                                           + PB(d, n, k - 1, L - 1)));
        } else {
            ...
            carry(d, n, k) = 0.0f;
            RDom rk(1, C - 1);
            carry(d, n, rk) = PA(d, n, rk - 1, L - 1) * carry(d, n, rk - 1)
                              + PB(d, n, rk - 1, L - 1);
        }
        ...
        Expr kk = t / L, jj = t % L;
        Expr acc = 0.0f;
        for (int nn = 0; nn < N; nn++) acc += c(nn, t) * h(d, nn, kk, jj);
        y(d, t) = acc;
        ...
        y.bound(d, 0, D).bound(t, 0, T)
         .split(d, do_, di, V).reorder(di, t, do_).vectorize(di).parallel(do_);
        for (Func f : {PA, PB}) {
    ...
    try {
        ...
        for (int dd = 0; dd < D; dd++) {
            for (int nn = 0; nn < N; nn++) hs[nn] = bp[nn] * xp[dd];
            float y0 = 0; for (int nn = 0; nn < N; nn++) y0 += cp[nn] * hs[nn];
            cy[(size_t)dd * T] = y0;
            for (int t = 1; t < T; t++) {
        ...
        for (int dd = 0; dd < D; dd++) for (int t = 0; t < T; t++) {
