...
int main(int argc, char **argv) {
    ...
    auto build = [&](bool inductive) -> Func {
        ...
        if (inductive) {
            h.reorder_storage(d, n, t)
             .compute_at(y, t).store_at(y, do_).fold_storage(t, 2).vectorize(d, V);
        } else {
    ...
    try {
        ...
        double tc = 0;
        {
            ...
            for (int d = 0; d < D; d++) {
                for (int nn = 0; nn < N; nn++) hs[nn] = bp[nn] * xp[d];
        ...
        for (int d = 0; d < D; d++) for (int t = 0; t < T; t++) yflat[(size_t)d * T + t] = ri(d, t);
