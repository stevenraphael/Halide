...
#
# Usage: HALIDE=/home/sraphael/Halide-rebase ./run_mamba_gpu.sh
set -u
HALIDE="${HALIDE:-/home/sraphael/Halide-rebase}"
INC="$HALIDE/distrib_install/include"
LIB="$HALIDE/distrib_install/lib"
export LD_LIBRARY_PATH="$LIB:/usr/lib/wsl/lib:${LD_LIBRARY_PATH:-}"
BIN=/tmp/mamba_gpu_bench
SRC="$HALIDE/apps/mamba/mamba_gpu_bench.cpp"

if [ ! -x "$BIN" ] || [ "$SRC" -nt "$BIN" ]; then
    echo "building $BIN ..."
    g++ "$SRC" -O3 -march=native -I"$INC" -L"$LIB" \
        -lHalide -lpthread -ldl -Wl,-rpath,"$LIB" -o "$BIN" -std=c++17 || exit 1
fi
